...
const MeetingTypeList = () => {
  ...
  const createMeeting = async () => {
    ...
    try {
      ...
      console.error(error);
      toast({ title: 'Không thể tạo cuộc họp' });
    }
  };
  ...
  return (
    <section className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
      <HomeCard
        ...
        title="Cuộc họp mới"
        description="Tạo một cuộc họp tức thì"
      ...
      {!callDetail ? (
        <MeetingModal
          ...
          title="Tạo cuộc họp đã lên lịch"
        ...
        <MeetingModal
          isOpen={meetingState === 'isScheduleMeeting'}
          onClose={() => setMeetingState(undefined)}
          title="Cuộc họp đã lên lịch thành công!"
          handleClick={() => {
            navigator.clipboard.writeText(meetingLink);
            toast({ title: 'Liên kết đã được sao chép' });
          }}
